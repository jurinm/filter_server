...
describe("filterMessages", () => {
  it("works on Boolean filter", () => {
    expect(filterMessages(messages, { field: "boolean", type: "boolean", operation: "eq", value: true })).toEqual([
      boolTrue,
    ]);

    expect(filterMessages(messages, { field: "boolean", type: "boolean", operation: "eq", value: false })).toEqual([
      boolFalse,
    ]);
  });

  it("String filter", () => {
    expect(filterMessages(messages, { field: "string", type: "string", operation: "eq", value: "string" })).toEqual([
      stringEqual,
    ]);

    expect(
      filterMessages(messages, { field: "string", type: "string", operation: "startsWith", value: "$Star" })
    ).toEqual([stringStartsWith]);

    expect(filterMessages(messages, { field: "string", type: "string", operation: "endsWith", value: "th$" })).toEqual([
      stringEndsWith,
    ]);

    expect(filterMessages(messages, { field: "string", type: "string", operation: "contains", value: "$$" })).toEqual([
      stringContains,
    ]);
  });

  it("Number filter", () => {
    expect(filterMessages(messages, { field: "number", type: "number", operation: "eq", value: 12 })).toEqual([
      numberEqual,
    ]);
  });

  it("Date filter", () => {
    expect(filterMessages(messages, { field: "date", type: "date", operation: "eq", value: dateMessage.date })).toEqual(
      [dateMessage]
    );
  });
